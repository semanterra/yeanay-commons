export class GeoStateData implements RawGeoState {
    ...
    constructor(raw: RawGeoState, stateStatic: StateStatic) {
        ...
        const chambers = _.mapValues<StaticChamberSet, Chamber>(stateStatic.chambers, (v:Chamber, k:ChamberId) => ({id: k, ...v}))
    ...
    get lowestDistrictType() {
        return this.unicameral ? upperDistrictType : lowerDistrictType
    }

    get districtTypes() {
        return [
            lowerDistrictType && !this.unicameral,
            floterialDistrictType && this.hasFloterial,
            upperDistrictType
        ].filter(Boolean)
    }
}
...
const stateStatic: StateStaticSet = {
    "ak": {
        "chambers": {
            "lower": {
                "name" : "House",
...
export const geoStates: { [key: string]: GeoStateData } = _.mapValues(rawGeoStates,
    (s => new GeoStateData(s, stateStatic[s.id]))
)
